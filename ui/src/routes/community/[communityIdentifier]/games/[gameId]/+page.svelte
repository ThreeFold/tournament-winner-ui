    ...
    <table class="grid">
        ...
        <tbody>
            {#each data.players as player}
                <tr>
                    <td>1.</td>
                    <td>{#if player.usernamePrefix !== null}<span class="prefix" >{player.usernamePrefix} | </span>{/if}{player.username}</td>
                    <td>{player.gameCharacters.slice(0, 2).map(c => c.name).join(", ")}</td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
